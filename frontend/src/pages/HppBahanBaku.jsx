...
const HppBahanBaku = ({ category }) => {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem(`hppBahanBaku_${category}`);
      if (saved) setBahanBaku(JSON.parse(saved));
      else setBahanBaku([]);
    } catch (e) {
      setBahanBaku([]);
    }
  ...
  useEffect(() => {
    localStorage.setItem(`hppBahanBaku_${category}`, JSON.stringify(bahanBaku));
  }, [bahanBaku, category]);
  ...
  const handleDelete = (id) => {
    if (window.confirm('Hapus bahan baku ini?')) {
      setBahanBaku(bahanBaku.filter(item => item.id !== id));
    }
  };
  ...
  const saveEdit = (id) => {
    setBahanBaku(bahanBaku.map(item => item.id === id ? { 
      ...item, 
      name: editForm.name, 
  ...
  const totalCost = bahanBaku.reduce((acc, curr) => acc + curr.price, 0);
    ...
    <div className="animate-fade-in" style={{ width: '100%', maxWidth: '1000px', margin: '0 auto', paddingBottom: '4rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '2rem' }}>
            ...
            <tbody>
              {bahanBaku.map((item, idx) => (
                <tr key={item.id}>
                  ...
                  <td>
                    {editId === item.id ? 
                      <input type="text" className="form-control" style={{ padding: '0.3rem' }} value={editForm.satuan} onChange={e => setEditForm({...editForm, satuan: e.target.value})} /> 
                      : item.satuan}
                  </td>
                  <td>
                    {editId === item.id ? 
              ...
              {bahanBaku.length === 0 && (
                <tr>
                  <td colSpan="6" style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Belum ada data bahan baku.</td>
                </tr>
              )}
            </tbody>
            {bahanBaku.length > 0 && (
              <tfoot>
                <tr style={{ background: 'rgba(255,255,255,0.05)', fontWeight: 'bold' }}>
                  <td colSpan="4" style={{ textAlign: 'right' }}>Total Estimasi Cost:</td>
        ...
        <div style={{ background: 'var(--surface-dark)', padding: '1.5rem', borderRadius: '0.5rem', border: '1px dashed var(--border-glass)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '4fr 2fr 2fr 3fr', gap: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label className="form-label" style={{ fontSize: '0.85rem' }}>Satuan</label>
              <input type="text" className="form-control" placeholder="Cth: pcs/project" value={newItem.satuan} onChange={(e) => setNewItem({...newItem, satuan: e.target.value})} />
            </div>
            <div className="form-group" style={{ marginBottom: 0 }}>
